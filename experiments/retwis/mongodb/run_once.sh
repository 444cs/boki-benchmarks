...
scp -q $BASE_DIR/mongo_setup.sh $MANAGER_HOST:/tmp/mongo_setup.sh
scp -q $BASE_DIR/init_mongo_rs.js $MANAGER_HOST:/tmp/init_mongo_rs.js
...
for HOST in $ALL_STORAGE_HOSTS; do
    ssh -q $HOST -- sudo rm -rf   /mnt/storage/mongo
    ssh -q $HOST -- sudo mkdir -p /mnt/storage/mongo
done
echo "2"
sudo ssh -q $MANAGER_HOST -- 'docker stack deploy -c ~/docker-compose-generated.yml -c ~/docker-compose.yml boki-experiment'
sleep 60
echo "after 2"
for HOST in $ALL_ENGINE_HOSTS; do
