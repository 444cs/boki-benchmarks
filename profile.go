...
func profileMongo(ctx context.Context, input *ProfileInput) (*ProfileOutput, error) {
	...
	rows, err = db.Query("SELECT * from users WHERE userID = ? LIMIT 1", input.UserId)
	if err != nil {
		return &ProfileOutput{
			Success: false,
	...
	//have to either use a struct or declare all these variables below
	//We had performed a similar query on create_users.go on table "users". But there it has only 2 columns
	if err := rows.Scan(&userId, &username, &password, &followers, &followees, &posts); err != nil {
		log.Fatal(err)
	}
	...
	if value, ok := followers.(bson.M); ok {
		output.NumFollowers = len(value)
	}
	if value, ok := followees.(bson.M); ok {
		output.NumFollowees = len(value)
	}
	if value, ok := posts.(bson.A); ok {
